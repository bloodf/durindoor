...
- Codex and GitHub Copilot dashboard config writers now create absent client files but refuse unreadable, malformed, or structurally invalid existing files, preserving other providers, MCP servers, policies, and ChatGPT OAuth tokens. Ports decolua/9router#3524 and #3525. Closes #603.
- Outbound proxy routing now keeps local-machine targets on-machine across environment and connection proxy settings, handles `Request` inputs, fails strict routes closed, rejects loopback relay forwarding, and rejects automatic redirects across proxy/relay trust boundaries. LAN targets remain proxyable. Ports decolua/9router#3517. Closes #606.
- Responses SSE-to-JSON and usage translation preserve normalized cache, reasoning, and other token detail objects; OpenAI reasoning tokens are billed once as part of output, and official OpenAI/O-series list prices are refreshed without changing fork GPT-5.6 tier prices or Kiro suffix resolution. Ports decolua/9router#3481. Closes #592.
