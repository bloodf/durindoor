...
- feat(website): publish the documentation at `/docs` on the public site. The website renders the repo-root `docs/` folder with Fumadocs, so the Markdown next to the code is still the source and GitHub still shows it. The docs pages use the dashboard's tokens, fonts, and sidebar styling, and the project icon. Search is served from `/docs-search` instead of `/api/search` because the demo patches `fetch` for every `/api/*` URL once a visitor opens it. The homepage links to the docs from the header, the quick start, the footer, and each feature card, in all six locales. Its copy was cut down: the comparison table is gone, repeated claims were merged, and it now names the three inbound request formats, the OAuth providers, Codex reset credits, realtime, the proxy timeline, and the MCP gateway. The homepage header stays on one row at every width: the language switcher is a globe with a two-letter code, the actions share one height, and the links fold into a menu below 1100px. The docs landing page has a hero with search, start-here cards, and a section grid; the sidebar carries section icons, and pages get breadcrumbs, an edit link, and previous and next cards.
...
- feat(dashboard): split API-key CRUD into `/dashboard/keys`, leaving endpoint URLs and remote-access controls on `/dashboard/endpoint`. The `Require API key` toggle remains beside tunnel controls because it gates tunnel activation, and tunnel security warnings keep an in-page link to that setting.
...
- feat(keys): group API keys, then filter and search the list. Groups are organizational labels created on the API Keys page; a key can belong to several (a CI key that is also a staging key), and selecting multiple groups in the filter shows keys in ANY of them. Search matches the key name only — group membership is what the filter is for. Groups carry no authority: access stays governed by the key's policy, allowed combos, and provider-account scope, and deleting a group removes the label and its membership rows while every key survives. A key's assignment is written inside the same transaction as its other edits, so a rejected or failed update never leaves the key changed with its groups stale.
- feat(dashboard): split the MCP Gateway into an Instances page and a Gateway Keys page (`/dashboard/mcp-gateway/keys`). Registering upstream servers and minting harness keys run on different schedules — instances are configured once and tested repeatedly, keys are minted per harness and revoked independently — but they shared one page and one loader, so every key action refetched the instance list and revoking a key meant scrolling past instance configuration. Each page now loads only what it renders, with one deliberate exception: the keys page still fetches instances because a key's grants name them, and the grants picker would otherwise be empty. The sidebar entry becomes a collapsible MCP Gateway group with Instances and Keys children, and each page links to the other.
- feat(api): make every management surface reachable with the application API key, so a client or agent can configure DurinDoor the way the dashboard does. The management REST routes (`/api/providers`, `/api/keys`, `/api/combos`, `/api/usage`, `/api/settings`, `/api/models`, MCP gateway CRUD and its OAuth connect flow, …) previously accepted only a dashboard JWT, the CLI token, or a loopback caller with login disabled, so a remote script had no credential that worked. They now accept `Authorization: Bearer <durindoor-key>` as well. Raw secret reveal (`/api/keys/{id}/reveal` and the gateway equivalent) is deliberately excluded so a leaked inference key cannot dump every other credential, and `/api/shutdown`, the database export/import routes, version updates, OAuth auto-import, and the local-only process routes stay session/CLI-only. The gateway's three protocol surfaces keep gateway-key auth, while its `client-metadata` and `callback` leaves stay off the management gate because neither carries a DurinDoor credential. Auth-critical settings keys are still stripped from an API-key-authenticated `PATCH /api/settings`, because that check demands a JWT or CLI token.
- feat(mcp): expand the control MCP server from 7 tools to 17, so an agent can manage combos, quotas, keys and settings — not just read providers and toggle connections. New tools: `list_combos`, `get_combo`, `create_combo`, `update_combo`, `delete_combo`, `quota_snapshots` (provider-reported remaining allowance), `refresh_quota` (force a live re-read for one connection), `list_api_keys` (management view with usage totals; the raw secret is stripped), `get_settings`, and `update_settings`. `model_list` takes an optional `kinds` filter so image, tts, embedding, stt, imageToText, rerank and video catalogs are reachable, defaulting to `llm` as before. `update_settings` always strips secrets, the connection-scoped auto-ping keys, and every auth-critical key, and rejects a patch left with nothing to write rather than silently succeeding. Combo validation moved into one shared module, so the REST routes and the MCP tools apply the same name rules, capability normalization, connection allowlist limits, and rotation invalidation. Control-endpoint auth gains the same local semantic the LLM endpoints use: when `requireApiKey` is off, a loopback caller may omit the credential; a remote caller always authenticates.

## Fixes

- fix(db): make the opt-in PostgreSQL cutover actually complete. Rollback now uses `currentDataFile()` / `currentBackupsDir()` instead of a literal `~/.9router` (which `path.join` does not expand). Cutover checkpoints WAL, refuses to flip if the snapshot copy fails, freezes live writes while the lock is held, quotes camelCase identifiers, TRUNCATEs before mirror, asserts `COUNT(*)`, copies `_meta` except `schemaVersion`, and writes `databaseEngine` on both engines. The PG adapter rewrites SQLite DML (`INSERT OR IGNORE`, `datetime('now')`, `COLLATE NOCASE`), treats PG as transactional for quota locks, strips `postgresUrl` from settings GET/PATCH/export, and keeps get/run/all synchronous via a worker so existing repos keep working. SQLite 018 and PG 018 now share the `api-key-groups` name; the cutover log is PG 019. Rollback requires `force: true` after a cutover because PG-era writes are not copied back.
- fix(translator): stop sending Claude a forced `tool_choice` that thinking models reject. Anthropic refuses `tool_choice` of type `any` or `tool` whenever thinking is active — a forced choice prefills the assistant turn, which cannot coexist with the thinking/tool-use response format — so agentic clients got `tool_choice: type "tool" and "any" are not supported for this model` instead of a reply. Fable 5.1 could never succeed: it cannot disable thinking, and an OpenAI-format client sending `tool_choice: "required"` is translated to exactly the shape Anthropic refuses, so every forced-tool request was a hard 400. Forced choices are now downgraded to `auto` whenever the request asks for thinking or the model always thinks, preserving `disable_parallel_tool_use` so a caller that wanted a single tool call still gets one. A forced choice survives only when thinking is genuinely off — the model can disable it *and* the request did not ask for it. The same guard previously existed only inside the GitHub Copilot executor, protecting one route while every other Claude-format provider still emitted the rejected body; it now lives in the translator and covers both the native and translated paths.
- fix(dashboard): stop the MCP Gateway grants picker treating a failed load as an empty one. Both the instance list and the key's current grants can fail to load, and each failure left an empty list that rendered as "No instances exist yet" or as a key granting nothing — with saving still enabled. Saving from either state would have written an empty grant list and revoked every instance the key could reach. Each failure now says so distinctly, an instances failure offers a retry, and saving is refused until both loads succeed.
- fix(dashboard): stop the MCP Gateway grants picker claiming no instances exist while they are still loading. Instances are fetched when the picker opens, so they race the picker's own grants request; the picker treated only its grants fetch as a load, so a fast grants response rendered "No instances exist yet" over a list that was merely in flight — telling the operator to create something that already exists. The picker now waits for both, and saving stays disabled until they settle so a save cannot commit grants chosen against a list that had not loaded.
- fix(dashboard): stop the MCP Gateway keys page reporting an instances outage it does not depend on. The page loaded instances on mount to populate the grants picker, so an instances endpoint failure raised an error toast even though the key list had loaded fine — and revoking a key is exactly when an unrelated error is most unwelcome. Instances are now fetched when the grants modal opens, which is the only moment they are needed.
...
- fix(quota): keep showing Claude quota during a rate-limit cooldown instead of blanking the card. Two gaps combined to hide the numbers. The in-process cache dropped an entry after 30 minutes while the 429 cooldown escalates to 2 hours, so the stale-fallback had nothing left to return; entries are now retained for 3 hours and every fallback path (cooldown, live 429, upstream 5xx) reads the retained entry rather than the serve-fresh one. Separately, a server restart empties that cache entirely, so a dashboard opened mid-cooldown received a message-only response even though the browser still held the last-known rows; those saved rows are now substituted for the same connection only, and only when the fresh response has none. Stale rows render with a visible reason so cached numbers are never mistaken for live ones.
...
- docs(site): rewrite every documentation page as MDX from the current code. Ten sections: getting started, providers, integrations, guides, features, deployment, operations, reference, contributing, plus FAQ and troubleshooting. The API reference lists all 39 `/v1` routes, the environment reference lists every variable the code reads, and the provider catalog is generated from the registry with `npm run gen:provider-catalog`. The management REST surface has its own reference page covering every `/api/*` route and the credentials each accepts, the MCP control server has a page listing every tool with client setup for Claude Code and Cursor, and a guide walks through automating DurinDoor with an API key. `npm run check:docs` now validates the MDX tree: every page must be listed in its folder's `meta.json`, carry `title` and `description`, avoid em dashes and emoji outside code, and use `./` or `../` for page links, since Fumadocs resolves nothing else. `scripts/docs-fact-diff.mjs` and `scripts/docs-claims-check.mjs` back the review process. CI builds the website and runs these checks on every pull request, and `cd website && npm run test:docs` crawls every docs route, exercises search after visiting the demo, and runs accessibility checks.
- chore(repo): remove files nothing referenced (`unused`, `captain-definition`, `start.sh`, `scripts/build-app.js`, `scripts/test-kimchi-ua.mjs`), the `plans/` directory, the port-campaign ledgers, the UI-migration playbooks, and the research notes under `docs/development/`. `AGENTS.md` is now a 53-line contributor contract. `CLAUDE.md` states the production port as 20128 and the data directory as `DATA_DIR` with a `~/.9router` default. The root `README.md` is a short front door that links into the site, and `DOCKER.md` moved to `docs/deployment/docker.mdx`.
- chore(deps): update runtime packages `@aws-sdk/client-bedrock-runtime` 3.1084→3.1132, `@xyflow/react` 12.11.2→12.11.6, `confbox` 0.2.4→0.3.1, `jose` 6.2.3→6.2.12, `marked` 18.0.6→18.0.13, `material-symbols` 0.45.7→0.47.3, `open` 11.0.0→11.0.4, `pxpipe-proxy` 0.9.0→0.13.2, `socks-proxy-agent` 8.0.5→10.1.0, `undici` 7.29.0→7.29.1, and `uuid` 13.0.2→14.0.2; update tooling `@oxlint/plugins`/`oxlint` 1.78.0→1.83.0, Storybook 10.5.10→10.6.0, esbuild 0.25.0→0.25.12, lint-staged 15.5.2→16.4.0, and postcss 8.5.23→8.5.28. The existing `pxpipe-proxy` `transformAnthropicMessages` API remains compatible. Defer Next.js/React updates beyond patch level and their coupled `eslint-config-next`/`react-is` versions; Monaco Editor 0.56, esbuild 0.28, ESLint 10, Vite 8, and plugin-react 6 require dedicated migrations; happy-dom 20.14 caused DOM overlay regressions in the full suite.
...
- port(upstream): f6e7cabe - stop sending ClinePass API keys as `workos:` bearer tokens, and refresh ClinePass on the proactive path. Every Cline credential was prefixed with `workos:`, which is correct only for Cline OAuth access tokens (WorkOS JWTs); a ClinePass API key (`clp_…`) is opaque and accepted verbatim, so the prefixed value came back as HTTP 401 "re-authenticate your Cline account". Only JWT-shaped tokens are prefixed now, and an existing prefix is never doubled. Separately, `cline`/`clinepass` had no entry in the proactive `REFRESH_HANDLERS` map, so scheduled refreshes fell through to the generic form-encoded OAuth grant that `api.cline.bot` rejects and expired tokens were never rotated; both providers now use the Cline JSON grant (`{ refreshToken, grantType, clientType }`) that the reactive executor path already spoke. ClinePass also lists `apikey` before `oauth`, because the OAuth extension flow does not mint tokens its consumer endpoint accepts.
- port(upstream): a7047a07 - restore the Codex `Version` header and single-source the CLI version. The identity the Codex backend sees was hardcoded in five places — the registry transport `User-Agent`, its `usage.clientVersion`, the image handler's `Version` and `User-Agent`, the usage and quota header builders, and the connection-test headers — all pinned to a CLI version (0.136.0) that OpenAI had long superseded. Every site now derives from one `cliVersion` on the codex registry transport, exported as `CODEX_CLI_VERSION` / `CODEX_CLI_USER_AGENT`, and the advertised version moves to 0.154.0.
- port(upstream): #3997 - keep refreshing connections whose `expiresAt` is a numeric epoch string. `parseTimeMs` accepted numbers and date-like strings, but `new Date("1789012345678")` is an Invalid Date, so a numeric epoch stored as text resolved to no expiry at all — `shouldRefreshCredentials` then returned false and the connection sat on an expired access token until it was re-authorized by hand. Both OAuth bulk-import routes persist the user-supplied `expires_at`/`expiresAt` verbatim, so the shape reaches the database. Numeric text is now converted numerically before the `Date` fallback, and `normalizeExpiresAt` routes through the same parser so an affected connection self-heals to ISO form on its next refresh.
- port(upstream): #4056 - stop injecting behavioral style prompts into requests that carry media. The caveman and ponytail token-saver prompts measurably degrade multimodal grounding on susceptible models — the model answers about an image it was told to be terse about — and a wrong answer is never worth the saved tokens. A new `hasMediaBlocks` scans the serialized request body for media markers in any wire format, including Kiro's `conversationState` envelope and images nested inside `tool_result` content, and both injectors now skip when it matches. Matching is anchored on `"type": "<media>"`, so a tool schema that merely names an `image` property does not disable the saver for ordinary text traffic.
- port(upstream): #3967 - add the Cline free-tier models and API-key auth. `cline` now accepts an API key as well as OAuth, matching its `clinepass` sibling, and lists the free tier (Muse Spark 1.3, DeepSeek V4 Flash, Solar Pro 4, LongCat 2.0, Laguna S 2.1) — usage-billed at $0 with a separate daily per-model allowance. Solar Pro 4 needs its own capability pattern ahead of the OpenAI o-series block: "solar-pro4" contains "o4", so the `*o4*` catch-all would otherwise advertise a text-only model as vision-capable with a 100K output ceiling. The pattern is scoped to `solar-pro4` so the existing provider-scoped `upstage.solar-pro3` override keeps carrying its own evidence.
- port(upstream): #4029 - store the Claude account email and real plan on OAuth connect. The quota card hardcoded the plan as "Claude Code" for every connection, so a Max 20x account and a Pro account were indistinguishable, and a Claude connection showed no email while Codex and Cursor ones did. The OAuth exchange now reads the account profile the existing `user:profile` scope already authorizes, and `claudePlanName` derives the display plan from `rate_limit_tier`, falling back to organization type and the account flags; a connection with no profile data still reads "Claude Code" exactly as before. Existing connections are backfilled in the background — that call is network-bound, so unlike the local codex/cursor backfills it never blocks the connection listing. The account email and display name join the fields the connection sanitizer already exposes; the org and account identifiers live under `providerSpecificData`, which the sanitizer never sends to the client.
- port(upstream): #3960 - add the Token Market provider. An OpenAI-compatible gateway with smart routing, registered as an API-key provider under the `tokenmarket` id and `tm` alias, with a five-model seed catalog and `passthroughModels` so ids added upstream stay usable without a registry change. Its thinking control is a provider-agnostic boolean rather than portable effort levels, so a new `tokenmarket` thinking format sets `extra_body.enable_thinking` and merges into any existing `extra_body` so routing controls such as provider sort preferences survive. Model discovery uses this fork's registry-level `modelsFetcher` convention rather than a hardcoded route entry.
- port(upstream): #4027 - clear credential fields when an auth modal closes. The Cursor and Kiro auth modals kept their token, API-key and CLI-proxy-JSON fields in component state with no reset, so cancelling left the secret populated and it was still rendered in the form the next time the modal was opened — visible to anyone who reopens it, and carried into an unrelated connection attempt. Both now clear on close, following the `isOpen`-transition pattern the provider Add-API-key modal already uses. That modal and the compatible-provider modal were already fixed here (#6499) and are unchanged.
- port(upstream): #4068 - correct Grok 4.5/4.6 and Claude Opus 4 token costs. Grok 4.5/4.6 fell through to the generic `grok-*` fallback ($0.50/$2.00) instead of their published $2.00/$6.00 rates, and had no long-context tier at all, so any request at or beyond xAI's 200k threshold was billed at roughly a quarter of its real cost. `claude-opus-4-20250514` carried `output: 25.00` against `reasoning: 112.50` — Anthropic lists Opus 4 at $15/$75, so output had been edited down without its reasoning rate following, leaving the row billing reasoning at 4.5x its own output. `calculateCostFromTokens` gains `longContextInclusive` because xAI applies the long-context tier to requests that *reach* the threshold, not only those exceeding it. Upstream's reasoning-token change was deliberately not taken: it removes the dedicated `reasoning` rate and bills reasoning at the plain output rate, while this fork already prices reasoning correctly as a subset of completion.
- port(upstream): #4087 - isolate video poll cooldowns and keep polls on their pinned account. Polls carry no model, and a model-less scope resolves to the account-wide lock, so one failing video poll cooled down chat and every other modality on that account. Poll health is now tracked under `videopoll:<provider>`, following the existing `webfetch:<provider>` precedent. Selection also gains `strictConnectionId`: a video job exists only on the account that created it, so when that account is missing, deactivated, or cooling down the poll now fails instead of silently retrying against a different account that cannot have the job and would be billed for it. The 404-must-not-mark-the-account half of the upstream patch was already fixed here by `shouldMarkAccountUnavailable`.
- port(upstream): #4041 - add a local AI Credits cutoff for GitHub Copilot connections. GitHub bills premium interactions as credits and does not stop at a user-defined ceiling, so a runaway client could spend real money with nothing in the router to stop it. A Copilot connection can now carry an `aiCreditLimit`; when GitHub-reported credit usage reaches it, requests on that connection are refused locally. The check runs at the top of `GithubExecutor.execute`, so the Anthropic shim, `/responses` and `/chat/completions` routes are all gated identically, and usage is cached briefly so a burst costs one upstream call. It fails closed: an unfetchable, unparseable or malformed reading blocks rather than silently behaving like no limit. The UI states plainly that cached readings, GitHub reporting lag and in-flight requests can overshoot the cutoff, so it is not a guaranteed spending ceiling. Upstream's unrelated quota-parsing rewrite was not taken; only `credits_used` is surfaced, since that is the one field the cutoff depends on.
...
- fix(dashboard): dev mode no longer loops with `ERR_TOO_MANY_REDIRECTS` after login (the removed legacy rewrite re-triggered the proxy on the rewritten path).
- fix(ui): every dashboard page now carries a title and description in the top bar (Provider Health, Playground, MCP Gateway, Auto-configure, MCP Help, API Docs, Headroom, PXPIPE were rendering an empty bar).
- fix(ui): Quota Tracker toolbar aligns with the page content instead of being pushed to the right, the loading card spans the full grid width, and the custom page-size input only appears when "Custom" is selected instead of duplicating the page-size select.

# 3.20.1

- fix(headroom): dev mode no longer 500s on every page. `src/proxy.js` (the Next 16 middleware convention) keeps an edge-server entry alive, so Next bundles `src/instrumentation.js` for the edge runtime too, and webpack follows every `await import(...)` it can parse regardless of runtime `if` guards. The Headroom autostart from d9c5594e therefore pulled `settingsRepo → db/driver → provider registry → os/fs/child_process/node:*/bun:sqlite` into the edge bundle, failing every compile with `Module not found` / `UnhandledSchemeError`. All Node-only boot work (process-title rename + Headroom proxy autostart) now lives in `src/instrumentation.node.js`, reached through a dynamic import inside `if (process.env.NEXT_RUNTIME === "nodejs")`; `NEXT_RUNTIME` is a compile-time literal per bundle, so the edge compiler constant-folds the branch away. Runtime behavior in the nodejs runtime is unchanged (autostart stays fail-open and non-blocking); regression tests pin the edge no-op and keep `instrumentation.js` free of unguarded imports.

# 3.20.0

- feat(ui): the dashboard is rebuilt on the Durin DS design system, behind a switchable preview. The previous dashboard remains the default; **Profile -> Local Mode -> Dashboard version** switches between `Old UI` and `New UI`, and both interfaces carry the switch so either choice can be reversed. The preference is per browser, stored in a `durindoor-ui-version` cookie read after the dashboard auth checks, so it cannot reach a page a reader could not already open. See `docs/development/dashboard-version-preview.md` for how to make the redesign the default and how to retire the preview.
- test(gate): bound the Vitest worker pool to 4 in `tests/vitest.config.js`. The default of `cpus-1` (15 on a 16-core host) let `quota-reservation-concurrency`'s 100 SQLite writer threads race other suites' `better-sqlite3` adapters, producing hook timeouts and literal `database is locked` errors that named a *different* set of "regressions" on every run of a byte-identical tree. Bounding workers made three consecutive full-suite runs pass with zero failures.
- fix(playground): stop emitting browser console errors for model discovery that cannot succeed. `/api/providers` now reports `canDiscoverModels`, computed from the same credential and resolver contracts the models route enforces, and the Playground skips the request instead of provoking a 401 (connection without a credential) or 400 (provider with no listing endpoint). Error handling for genuine failures is unchanged; no console output is suppressed.
- fix(a11y): dashboard accessibility repairs across 49 files - scrollable `pre`/log/list regions are keyboard focusable with accessible names, icon-only controls (priority arrows, model test/copy/edit/delete, availability refresh, combo judge and drag handles) carry real labels, and `aria-label` on non-semantic spans is either given a valid role or removed.
- test(e2e): grant clipboard permissions to the Storybook evidence context for Chromium only, so stories exercising `navigator.clipboard.writeText` no longer fail with `NotAllowedError` from an environment gap rather than a product defect.
- test(e2e): retain axe node target selectors and persist a `failures/<story>-failure.json` diagnostic before rethrowing, so failing evidence cases are diagnosable. Passing-case artifact bytes are unchanged, preserving their verified sha256.
...
- port(upstream): #3772 - replace nested Claude `tool_result` base64 media with a bounded media-type placeholder; ordinary top-level user images are unchanged. Ports the image-blob half of decolua/9router#3772 while the Responses parallel tool-call index half is tracked separately. (open-sse/translator/request/claude-to-openai.js)
...
- add(catalog): gpt-6-astra is now a built-in default for the direct OpenAI API and the ChatGPT Codex OAuth surface. The OpenAI API entry resolves 1.05M context / 922K max input / 128K output; Codex resolves 272K context with no published output ceiling. The request sanitizer strips unsupported `temperature`, `top_p`, `top_logprobs`, and Chat `logprobs`; Responses removes only `include: ["message.output_text.logprobs"]`. Reasoning effort accepts `low/medium/high/xhigh/max` on the API and adds `ultra` on Codex; Astra cannot disable reasoning, so client `none` and the unsupported `minimal` value are rewritten to the published `low` floor on the wire. The API list price is added to `PROVIDER_PRICING.openai` with a 272K long-context tier (2x input/cache, 1.5x output); Codex has no published billing override. Existing saved `connection.defaultModel` values are preserved unchanged. Source: [OpenAI model docs](https://developers.openai.com/api/docs/models/gpt-6-astra), [pinned openai/codex `codex-rs/models-manager/models.json`](https://github.com/openai/codex/blob/3921a30d6b11218883e5bb81a48082095cf79b7c/codex-rs/models-manager/models.json).
- port(upstream): #3785 - DeepSeek Chat requests with malformed non-array `messages` now pass through the reasoning injector unchanged, allowing normal downstream validation instead of throwing a `body.messages.map is not a function` error.
...
- port(upstream): #3654 - selective provider/combo transfer. `POST /api/settings/database/selective` provides dual-auth catalog, preview, export, and apply actions. Preview and default export omit credentials and credential-bearing URLs; acknowledged secret exports include only encrypted-at-rest top-level credential fields. Imports validate provider/combo shapes and invariants before one transaction, preserve omitted credentials, and retain unrelated rows.
- port(upstream): #3747 - compact token display via a shared `src/shared/utils/formatCompact.js` formatter used by OverviewCards, UsageTable, and UsageStats' rendered RequestsPanel (token totals ≥ 1,000 switch to compact notation such as 1.2M while the exact locale total stays in a title tooltip). Settings route now exposes the fork's resolved `enableObservability` (`OBSERVABILITY_ENABLED` env first, then the stored `enableObservability` setting) instead of the upstream `ENABLE_REQUEST_LOGS`, and Usage → Request Details shows a settings-linked empty-state callout when observability is off. No raw palette markup, no native dialogs, no `window.prompt`/`window.confirm`, no `<select>` introduced.
...
- port(upstream): #3768 - per-member combo weights. Combos now persist `members: [{ id, weight }]` alongside existing `models` string list (`src/lib/db/repos/combosRepo.js`, migration 015, `SCHEMA_VERSION=15`). New `Weighted` strategy uses Efraimidis–Spirakis weighted random permutation through existing rotation seam. Weights require positive finite numbers; malformed members return HTTP 400. Legacy string-only combos remain weight 1.
- port(upstream): #3768 - per-member combo weights. Combos now persist `members: [{ id, weight }]` alongside existing `models` string list (`src/lib/db/repos/combosRepo.js`, migration 015, `SCHEMA_VERSION=15` at this stack boundary). New `Weighted` strategy uses Efraimidis–Spirakis weighted random permutation through existing rotation seam. Weights require positive finite numbers; malformed members return HTTP 400. Legacy string-only combos remain weight 1.
- port(upstream): #3748 - connection groups, combo connection allow-list enforcement, and future-only usage attribution/reporting (issue #747, Units 1–5). Operators can create, edit, delete, and manage membership for named connection groups in the dashboard, then use those groups when configuring a combo's explicit connection allow-list (`combos.allowedConnectionIds`). The shared credential-selection seam (`getProviderCredentials`) enforces that allow-list before locks, quota preflight, RPM, fallback, and any noauth/optional-noauth escape, across every combo-enabled modality (chat, fetch, search, image generation, TTS). An empty/unset allow-list preserves unrestricted behavior; a populated list with an empty intersection denies dispatch, and nested fusion/auto combo allow-lists intersect rather than widen. New usage records persist the outer client-selected combo id/name and selected connection; Usage reports attributed rows by combo/connection while keeping historic and direct usage explicitly unattributed instead of backfilling it. Native Ollama `prompt_eval_count`/`eval_count` values are normalized into input/output token counts on the real chat-core attribution path. Does NOT port upstream's unsafe provider-key export endpoint (deferred, separate security review required — issue #747 Unit 6).
...
## Added
- API key provider-account scoping: operators can restrict each API key to a specific subset of provider accounts via a new `apiKeyProviderConnections` join table (migration 014). Zero relation rows keeps the key unrestricted (zero-row invariant), and `apiKeys.policy.allowedModels` keeps working unchanged. Create/update routes accept `providerConnectionIds` and run the key write and join replace in one SQLite transaction; provider-connection ids are validated against the live `providerConnections` table inside that transaction. The provider connection and provider-node delete paths reject the deletion with HTTP 409 `API_KEY_SCOPE_WOULD_BROADEN` when a key would otherwise be silently widened, enforced at the repo layer so the protection covers every caller. Endpoints `GET /api/keys`, `GET /api/keys/[id]`, `POST /api/keys`, and `PUT /api/keys/[id]` are documented in `docs/security/api-key-provider-account-scoping.md`. Ports decolua/9router#3661 without its unenforced `connections` scope or its `apiKeys.scope` field.

- Explicit combo capability ceilings (closes #761). Every combo can carry an optional, operator-defined `capabilities` ceiling (booleans + `contextWindow` / `maxOutput`) that overlays the existing member-derived `aggregateComboCapabilities` via intersection (member-safe minimum). Booleans can only be disabled, numeric limits can only be lowered, and an explicit cap cannot enable a capability the derivation would not already permit. Disabling `reasoning` clears its thinking metadata. `/v1/models`, dashboard combo cards, model pickers (`useModelCaps` hook), nested combo resolution, and `POST` / `PUT /api/combos` now reflect the effective capabilities; unknown keys, non-booleans, and non-positive safe-integer limits return HTTP 400. Empty caps never disable by omission. Persistence extends the `combos` table with a new `capabilities TEXT` column (migration `014-combo-capabilities`); legacy combos without a cap keep their full member-derived capabilities. Port of decolua/9router#3754 (caps field renamed to `capabilities` to match fork convention; semantics adapted to intersection over derivation, not replace).
...
- AI Horde (`aihorde`, alias `horde`) is now a free no-auth provider through the official OpenAI-compatible facade. DurinDoor uses AI Horde's anonymous sentinel by default, lets saved API keys replace it, and discovers the current volunteer-worker model catalog live without fabricating model capabilities or token limits.
...
- Wrong-model errors are now terminal before 401/404 cooldown handling, so a provider-reported `ModelError` no longer cools healthy accounts. Client responses preserve stored 400/422/401/403/503 classes through chatCore and exhausted fallback paths; only a 401 `ModelError` that explicitly says its model is missing is normalized to 404.
- Combo fallback now recognizes normalized web-fetch responses with non-empty `content.text` as successful while continuing to retry and fall back for empty text or arbitrary content objects.
...
- OpenAI `response_format: { type: "json_schema", json_schema: { schema } }` requests now map the supplied JSON Schema into Gemini `generationConfig.responseSchema` and set `generationConfig.responseMimeType` to `application/json`. Nullable unions (`type: ["T", "null"]`, `anyOf`, `oneOf`) become Gemini's `nullable: true` form while keeping the non-null winner and nested `properties`/`items`; caller bodies remain unchanged and existing tool-parameter cleaning remains unchanged. Gemini 3 models support this together with emitted function declarations; Gemini 2.5 and unknown tool-bearing requests preserve tool mapping but omit both structured-output fields to avoid the documented 400. Tool-free requests retain structured output for every model. Antigravity does not support structured output, so its route strips both fields regardless of model/tools while retaining other generation config and tools.
- Codex reset-credit failures now surface nested upstream error messages and serialize other structured errors instead of reporting `[object Object]`. Ports decolua/9router#3778.
- Saved combo resolution now excludes disabled members before dispatch. Each saved member is matched by its own provider form (registry id or registry alias, whichever it was saved under) plus that same provider's other static alias, a resolved model alias's target provider, and — for openai/anthropic-compatible connections — both the connection's storage id and its configured output prefix; a disabled row keyed by a different provider's id never reaches an unrelated saved member. Non-string legacy entries (`null`, numbers, `{kind:'combo-ref'}`, `{providerId,model}`) are preserved unchanged. A combo whose members are all disabled resolves to `[]` (existing terminal 503), not `null`; a genuine DB miss remains `null`. Disabled state is read from the existing disabled-models repository and never conflated with connection `isActive`. Fail-open behavior is per read: a `getDisabledModels` rejection returns the original member array unfiltered; a `getModelAliases` rejection leaves bare model-alias members unresolvable and therefore kept; a `getProviderNodes` rejection still matches members on their literal prefix but loses the storage-id↔output-prefix cross-reference.
...
- Ollama Cloud connections now support `ollama/fetch` through the documented web-fetch API with guarded requests, normalized/truncated content and links, and fetch-only account cooldowns. Ports decolua/9router#3624 while retaining DurinDoor's callback-free success flow and `webfetch:ollama` lock scope.
...
- Gemini, Vertex, and Antigravity now preserve distinct long tool names across declarations, forced choices, history, and response restoration instead of collapsing common 64-character prefixes. Ports open decolua/9router#3637 with DurinDoor's response-aware SHA-256 aliases rather than upstream's mismatched 32-bit MD5 map. Closes #675.
...
- Claude Code model discovery now exposes reversible Claude-shaped IDs for every routable DurinDoor model, decodes recognized `[1m]` requests before routing, and echoes the exact client-selected alias in Anthropic streaming metadata. Ports the compatibility boundary from open decolua/9router#3595, #3691, and #3693 while reusing DurinDoor's existing model-limit, alias/combo, and response-finalization seams instead of upstream's second context store, per-request DB index, dashboard/statistics rewrite, and deploy bundle. Closes #669.
...
- Public inference, media, batch, file, and catch-all responses now carry a trusted server-owned `x-request-id`; JSON errors repeat it as `request_id`, validated provider IDs remain separate as `upstream_request_id`, and relay traffic no longer trusts client-supplied correlation. Ports the correlation half (`#3689b`) of open decolua/9router#3689 across DurinDoor's full handler inventory while preserving conversation-stable provider affinity, Codex session identity, and prompt-cache keys. Closes #689.
...
- OpenAI passthrough streams now emit one correctly framed `[DONE]`, suppress repeated terminal chunks per choice, and mirror `delta.reasoning` into missing `reasoning_content` while preserving both fields and existing canonical values; non-streaming and forced-SSE-to-JSON paths now reject HTTP-200 native gateway errors while preserving valid tool calls. Ports open, unmerged decolua/9router#3611 while retaining DurinDoor's existing explicit stream-mode resolution, provider normalizers, and per-choice terminal tracking. Closes #667.
...
- Custom OpenAI-compatible and Anthropic-compatible nodes now expose only persisted **Available Models** rows and compatible aliases through `/v1/models`; stale `enabledModels`, static fallback rows, live discovery, disabled provider-id rows, and duplicate raw provider-id entries cannot advertise unapproved IDs. Ports open decolua/9router#3623 into DurinDoor's extracted model-list builder while preserving dashboard import discovery, registry-backed fetchers, Kimi live catalogs, and local passthrough discovery. Closes #670.
- Exact provider-prefixed `GET /v1/models/{provider}/{model}` lookups now return the configured OpenAI model object or `model_not_found`, while capability and `rerank` lists, local-first provider validation, response projections, cheap `HEAD`, CORS methods, and legacy unknown one-segment errors remain intact. Ports open decolua/9router#3649 with DurinDoor-specific compatibility and safety behavior. Closes #677.
- Codex Spark requests now omit unsupported `reasoning.summary`, recovering HTTP 400 `unsupported_parameter` failures, and regular Responses requests carry one compaction policy capped at 100K tokens. Ports open decolua/9router#3664 while using DurinDoor's immutable `requestContext.compact` to keep `context_management` out of standalone compact calls.
- Reloads now apply valid persisted light, dark, and system themes before first paint, preventing a light flash without changing DurinDoor's Zustand theme state or CSP policy. Adapts open decolua/9router#3660 to the fork's persisted envelope and same-origin script authorization.
