...
function buildMenuItems(port, autostartEnabled) {
  return [
  { title: `DurinDoor (Port ${port})`, tooltip: "Server is running", enabled: false },
...
function handleClick(index, options, onAutostartToggle) {
  ...
  if (index === MENU_INDEX.DASHBOARD) {
    if (onOpenDashboard) onOpenDashboard();else
    ...
    try {
      ...
      if (enabled) disableAutoStart();else
      enableAutoStart();
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[durindoor] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[durindoor] tray init error: ${err.message}\n`);
    return null;
  }
}
...
function killTray() {
  ...
  let proc = null;
  try {
    proc = instance._process || (isFunction(instance.process) ? instance.process() : null);
  } catch (e) {}
