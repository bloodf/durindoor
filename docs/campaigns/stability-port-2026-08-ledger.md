...
| PR | Source | Title | Verdict | Evidence / commit |
|----|--------|-------|---------|-------------------|
| #3018 | 9router | fix(translator): drop JSON Schema keywords Gemini has no field for | GAP | `open-sse/translator/formats/gemini.js:5` — `UNSUPPORTED_SCHEMA_CONSTRAINTS` covers 15 keywords; 5 advanced MCP-tool keywords missing: `uniqueItems`, `contains`, `unevaluatedProperties`, `unevaluatedItems`, `contentSchema`. Port recipe: append to existing array. |
...
| #1425 | 9router | Default Codex reasoning to medium | GAP | `codex.js:864` — `resolveOpenAiEffort(body.reasoning_effort || modelEffort || 'low', ...)` hardcoded `'low'`; upstream uses `CODEX_DEFAULT_REASONING_EFFORT = "medium"`. Port recipe: constant `'low'` → `'medium'`. |
...
| #1337 | 9router | Fix Xiaomi reasoning content echo | GAP | `reasoningContentInjector.js:11-14` — only `deepseek` provider and `kimi-` model rules; no Xiaomi entries. Port recipe: add `xiaomi-mimo` and `xiaomi-tokenplan` to `MODEL_RULES`. |
