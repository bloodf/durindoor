...
| #1821 | 9router | fix(codex): treat OpenAI 401/token_expired as permanent refresh failure | GAP → ported | `open-sse/services/tokenRefresh/providers.js:301-309` now detects OpenAI HTTP 401 + `token_expired`/`could not validate your token` markers and returns `permanent:true`; regression in `tests/unit/oauth-classify-token-expired.test.js` (commit 4fa1153e9). |
