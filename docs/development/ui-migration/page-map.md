...
| 5 | `/dashboard/endpoint` | `src/app/(dashboard)/dashboard/endpoint/page.js` + `EndpointPageClient.jsx` | `src/shared/ui/pages/endpoint/EndpointPage.jsx` | `PageHeader`, `Card`, `Button`, `Toggle`, `IconButton` | medium | Endpoint URLs, Cloudflare/Tailscale controls, and the API-key requirement toggle | pending |
| 5a | `/dashboard/keys` | `src/app/(dashboard)/dashboard/keys/page.js` + `KeysPageClient.jsx` | no mock | `PageHeader`, `Card`, `Input`, `Select`, `Button`, `DataTable`, `ConfirmDialog` | medium | API-key CRUD, access policies, expiry, quotas, and provider-account restrictions | pending |
