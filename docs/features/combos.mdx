...
| `fusion` | Query every member in parallel as a panel, then a judge model writes one answer. Fusion has its own panel timeout, not the per-member timeout. Best quality, highest cost: each request bills every panel member plus the judge (N+1 calls). |

Capacity auto-switch: with capability-aware routing on, a request that carries images, PDF, or audio goes first to a member that supports that input, whatever the strategy. See [Capability ceiling](#capability-ceiling).
...
the server URL (default `http://127.0.0.1:8000`; `host:port` without a scheme
means `http://`), and add an API key only if the server was started with
`LAYA_API_KEY`. A saved URL that is not http(s) is refused rather than replaced
by the default host, and Jev keeps classifying. While a Laya connection is active,
