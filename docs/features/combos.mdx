...
looks, then puts the best match first. The rest of the combo keeps its order, so
the fallback ladder is unchanged. Capability-aware routing still wins: a request
carrying images prefers a member that advertises vision even when the complexity
score would have picked another member.
...
Only the current user turn is sent, capped at 4000 characters, never the whole
transcript and never tool results. The call has a 3s deadline and a 30s circuit
breaker. It fails open: no key, a timeout, an HTTP error, an unparseable body, an
