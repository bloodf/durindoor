---
title: MCP gateway
description: One JSON-RPC endpoint in front of registered MCP servers, with gateway keys, grants, and OAuth.
---
...
Open **Dashboard → MCP Gateway → Instances** (`/dashboard/mcp-gateway`) to register servers. Open **Dashboard → MCP Gateway → Keys** (`/dashboard/mcp-gateway/keys`) to mint keys. Client configuration is under [Client setup](#client-setup) below.
...
Stdio bridge: allowlisted local plugins run as a child process on the DurinDoor host and are bridged to SSE. The bridge only spawns plugins on that allowlist, never a command taken from a request. Stdio instances (`npx`, `python`, `docker`, `command` kinds) run the command saved on the instance.
...
Grant the key to instances from the Keys page. The tools a client sees are exactly the tools exposed by its granted instances. Removing a grant revokes access right away. An empty grant list means the key authenticates but sees no tools.

The control endpoint `POST /api/mcp/control` does not accept a gateway key. It takes a dashboard session, the CLI token, or a DurinDoor API key. Full tool list and client setup: [MCP control](./mcp-control.mdx).
...
`POST /api/mcp/control` is a separate JSON-RPC server for the running DurinDoor instance (combos, connections, settings, quota). Gateway callers stay on `/api/mcp-gateway` with a gateway key. Auth, every tool, client setup, and limits: [MCP control](./mcp-control.mdx). REST families and exclusions: [Management API](../reference/management-api.mdx).
...
- **URL not allowed.** The outbound URL guard rejected the upstream URL, for example a private address.
