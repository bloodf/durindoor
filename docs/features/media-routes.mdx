...
- It comes from a provider with an active connection, or from an enabled keyless provider that is installed and working:
  - A self-hosted keyless server (Local Whisper, Coqui, Tortoise, SearXNG, self-hosted Firecrawl, or any loopback or private URL) counts only while it answers HTTP.
  - `local-device` counts only when the OS voice list loads (macOS `say` or Windows SAPI).
  - Keyless libraries and public services (edge-tts, google-tts, veoaifree-web) count as working.
  - Checks are cached for 30 seconds.
