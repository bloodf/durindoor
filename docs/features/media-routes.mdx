...
- It comes from a provider with an active connection, or from an enabled keyless provider that is installed and working:
  - A self-hosted keyless server (Local Whisper, Coqui, Tortoise, SearXNG, self-hosted Firecrawl, or any loopback or private URL) counts only while it answers HTTP. For Local Whisper and self-hosted Firecrawl, it counts when the default URL or any saved connection's URL answers.
...
- A saved model that later becomes unavailable (connection removed or disabled, model hidden) is skipped and shown as **unavailable, skipped**. If every saved model is unavailable, the request fails with `no_provider_for_kind`. DurinDoor does not quietly switch to the automatic order. One exception: if the saved models are still available but the endpoint can't run any of them, the endpoint uses its own automatic list. Examples are a `veoaifree-web` video route on `/v1/videos`, or a Deepgram-only speech-to-text route on `/v1/audio/translations`.
