...
- It comes from a provider with an active connection, or from an enabled keyless provider that is installed and working:
  ...
  - For an unscoped request, Local Whisper uses its default host. Self-hosted Firecrawl uses the dashboard setting, then `FIRECRAWL_BASE_URL`, then its default. For an API key scoped to provider accounts, the provider counts if any connection that key may use answers.
