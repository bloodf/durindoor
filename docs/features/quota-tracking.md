...
The first persistence batch did not change account selection, fallback, combo scoring, quota fetches, dashboard surfaces, or request accounting. The request preflight and fallback consumer is now described below; later combo, accounting, and management consumers must continue to use this same contract rather than creating parallel caches or truth sources.
