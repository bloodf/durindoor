...
The host bind path `$HOME/.durindoor` is not the native default (`~/.9router`). Inside the container, `DATA_DIR` is `/app/data`. Pin a version tag in production. Full compose, TLS, and volume notes live in the [deployment](../deployment) section.
...
Back up `DATA_DIR` first. Then follow [Upgrading](../operations/upgrading.md). Global npm installs update with `npm update --global durindoor`.

## Migration from 9router

DurinDoor can reuse a 9router data directory. Stop the old process, back up that directory, start DurinDoor with the same `DATA_DIR`, then check providers, keys, combos, and usage in the dashboard.
