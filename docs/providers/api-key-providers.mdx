...
Saving a profile-only connection skips the usual API-key requirement, so `POST /api/providers` and the validate probe both accept an empty key. The validate probe still has no credential to send, so such a connection saves with test status `unknown`; test it from the provider page after `aws sso login`.
