...
The session token is stored in its own encrypted connection field, like the API key, and no provider API response returns it. Editing a connection with the session token field left blank keeps the saved token, unless you change the Access Key ID or clear it. Typing a new API key in the edit form clears a saved profile, since the profile would otherwise keep taking precedence. The edit form only saves that change after the new credentials pass the check, and the check reuses the saved session token when the Access Key ID is unchanged. A session token sent inside `providerSpecificData`, as 9router clients did, is moved into the encrypted field.

A missing or expired SSO session is reported as HTTP 401 `auth_error`, so run `aws sso login` again when you see it.

A temporary access key (`ASIA...`) saved without a session token is rejected up front with HTTP 401 rather than failing later as a signature mismatch.
