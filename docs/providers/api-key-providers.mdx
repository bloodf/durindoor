...
OAuth client overrides (`GEMINI_OAUTH_CLIENT_ID`, `ANTIGRAVITY_OAUTH_CLIENT_ID`, and their secrets) belong on [Connecting accounts](connecting-accounts.mdx). The full operator list is [Environment variables](../reference/environment.md).
...
Details, deletion guards, and the HTTP shape live in [API-key provider-account scoping](../security/api-key-provider-account-scoping.md). Deleting a connection that is the last scoped account for a key is rejected with HTTP 409 `API_KEY_SCOPE_WOULD_BROADEN` so the key cannot silently become unrestricted.
