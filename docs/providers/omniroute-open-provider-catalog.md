...
- OmniRoute PR #6311: API-key connection creation now defaults to `main`, then `main-2`, `main-3`, and so on. DurinDoor still keeps the backend name-based upsert behavior, but the add-connection modal no longer silently reuses `main` for a second API-key connection.
