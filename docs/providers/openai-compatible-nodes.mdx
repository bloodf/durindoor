...
OpenAI-compatible and Anthropic-compatible connections have no fixed billing endpoint, so DurinDoor cannot show Provider Limits for them out of the box. Set `providerSpecificData.quotaEndpoint` on the connection (via `PATCH /api/provider-connections/:id`) to describe where the quota lives and how to read it:
...
`coding-default` is a combo or a model alias, not a node prefix. Model aliases are `PUT /api/models/alias` with `{ model, alias }` and show up as extra names for an existing model string. Combos are a fallback chain; see [Combos](../features/combos.mdx).
