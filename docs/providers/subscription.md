...
| `chatgpt-web` | Ported chat executor. | Normalizes ChatGPT session-token cookies, exchanges `/api/auth/session` for a bearer token, scrapes the ChatGPT DPL/script prekey inputs, calls Sentinel chat requirements, computes the required PoW token, builds ChatGPT `/backend-api/f/conversation` chat bodies, and converts ChatGPT SSE. Caller-supplied `providerSpecificData.chatgptWebSentinel` tokens remain supported as an override. Automatic Turnstile solving, TLS sidecar parity, and image/cache routes are not ported in this JS branch; Sentinel or upstream anti-bot failures surface as explicit upstream errors instead of a `provider_port_pending`/501 guard. |
