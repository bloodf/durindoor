...
| --- | --- | --- |
| `adapta-web` | Web Cookie LLM provider, alias `adp-web`, Adapta auto model. | Ported chat executor hashes the full Clerk `__client` cookie for session-JWT caching, strips optional `Cookie:` prefixes, honors per-connection proxies, and keeps validation tests covering Clerk credential exchange and OpenAI response conversion. Distinct Adapta backend aliases stay hidden until verified upstream model ids are available. |
| `chatgpt-web` | Web Cookie LLM/image provider, alias `cgpt-web`, ChatGPT model catalog. | Chat session exchange, Sentinel requirements/PoW, request conversion, SSE conversion, per-connection proxy support, and refreshed-cookie `apiKey` persistence are ported. Caller-supplied `providerSpecificData.chatgptWebSentinel` tokens remain supported as an override. Remaining blockers are automatic Turnstile solving, TLS-sidecar parity, and image cache parity; Sentinel or upstream anti-bot failures surface as explicit upstream errors instead of the old 501 port-pending guard. |
...
| `t3-web` | Web Cookie LLM provider, alias `t3chat`, T3 model catalog. | Ported chat executor probes authenticated `/api/chat`, sends `convexSessionId`, `threadMetadata`, response ids, and text parts, honors per-connection proxies, and keeps validation tests covering `convex-session-id` parsing and OpenAI response conversion. |
