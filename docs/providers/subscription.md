...
- The Codex OAuth catalog adds GPT-5.6 image aliases `cx/gpt-5.6-sol-image`, `cx/gpt-5.6-terra-image`, and `cx/gpt-5.6-luna-image` alongside the existing GPT 5.5, 5.4, and 5.3 image aliases. The image adapter strips `-image` and sends the underlying model an `image_generation` tool through the Codex Responses API; the same endpoint accepts an `image` reference for edits, and availability of each underlying model depends on the connected ChatGPT Plus or higher account.
