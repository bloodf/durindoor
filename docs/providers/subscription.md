...
| --- | --- | --- |
| `adapta-web` | Ported executor. | Uses the pasted Clerk `__client` cookie, strips optional `Cookie:` prefixes, hashes the full client token for the session-JWT cache, sends Adapta chat requests through the selected connection proxy when configured, and converts streaming or non-streaming text responses to OpenAI chat completions. Only the verified `adapta-one` auto model is advertised until distinct upstream model ids are confirmed. Image/cache flows are not advertised. |
| `chatgpt-web` | Ported chat executor. | Normalizes ChatGPT session-token cookies, exchanges `/api/auth/session` for a bearer token, scrapes the ChatGPT DPL/script prekey inputs including relative Next.js script URLs, calls Sentinel chat requirements, computes the required PoW token, builds ChatGPT `/backend-api/f/conversation` chat bodies, persists refreshed cookie chunks back to `apiKey`, honors per-connection proxies, and converts ChatGPT SSE. Caller-supplied `providerSpecificData.chatgptWebSentinel` tokens remain supported as an override. Automatic Turnstile solving, TLS sidecar parity, and image/cache routes are not ported in this JS branch; Sentinel or upstream anti-bot failures surface as explicit upstream errors instead of a `provider_port_pending`/501 guard. |
| `t3-web` | Ported executor. | Parses full t3.chat Cookie headers or `convexSessionId` structured input, probes the authenticated chat endpoint during validation, sends session-bound chat bodies with `convexSessionId`, `threadMetadata`, response ids, and text parts through the selected connection proxy when configured, and converts JSON, SSE, NDJSON, or TSS-shaped text responses to OpenAI chat completions. Convex/browser endpoint discovery beyond the `/api/chat` path is not ported. |
...
| `chatgpt-web` | Web Cookie LLM/image provider, alias `cgpt-web`, ChatGPT model catalog. | Chat session exchange, Sentinel requirements/PoW, and SSE conversion ported; remaining blockers are automatic Turnstile solving, TLS-sidecar parity, and image cache parity. |
