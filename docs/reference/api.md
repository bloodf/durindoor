...
Every inference, media, batch, file, and `/v1/*` catch-all response includes an `x-request-id` generated by DurinDoor for that request. Client-supplied `x-request-id` values are ignored and cannot select the downstream or relay correlation identity.

JSON errors repeat the same trusted value at the error envelope's correlation level: `error.request_id` for OpenAI-shaped `{ error: {...} }` responses, and top-level `request_id` for Anthropic `{ type: "error", error: {...} }` and flat error bodies. A strictly validated upstream provider identifier is exposed separately at the matching level as `error.upstream_request_id` or top-level `upstream_request_id`; it never replaces the server request ID. Successful JSON, binary, and streaming bodies are passed through without buffering for correlation.

Request correlation is per request. Conversation session identity, provider affinity, and provider cache/session fields remain stable across turns and are not derived from `x-request-id`.
