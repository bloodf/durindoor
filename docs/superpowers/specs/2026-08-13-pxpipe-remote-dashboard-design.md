...
The Token Saver client stores that error body as PXPIPE status without checking `response.ok`. Because the body has no `installed` property, the existing `!status.installed` branch falsely renders `Not installed` and recommends reinstalling the application.
...
`refreshPxpipeStatus` will check the HTTP response before replacing status. A failed request will preserve an explicit unavailable/error state rather than manufacturing `installed: false`.

A small pure PXPIPE status-view helper beside `TokenSaverClient` will classify the API state so the UI and unit test share one contract:
...
The card's dependency warning and disabled toggle will use an exact `installed === false` check rather than treating missing status data as proof that the package is absent. The health response remains separate and continues to show its own actionable diagnostics.
