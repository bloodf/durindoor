...
/**
 * Classify a Codex SSE error event by parsing the JSON data payload.
 * Returns "account" if the human-readable message matches capacity patterns,
 * "retry" if the error type matches retry patterns, or null if neither.
 * This avoids false positives from substring matches on JSON envelope keys
 * (e.g. "code":"model_at_capacity" should not pre-empt a "message":"…capacity…" parse).
 */
function classifyCodexErrorEvent(text) {
  ...
  async _peekSseTransientError(response) {
    ...
    try {
      while (text.length < CODEX_SSE_PEEK_BYTES) {
        ...
        // Fallback: substring match on lowerText for non-JSON / free-form SSE
        // bodies. Skip model_at_capacity here (JSON envelope key) to avoid the
        // split-event early-match issue; rely on the message field instead.
