  ...
  async wsChat(input) {
    return new ReadableStream({
      start(controller) {
        ...
        const timeout = setTimeout(() => abort("Microsoft 365 Copilot WebSocket timeout"), FETCH_CONNECT_TIMEOUT_MS);
        ...
        try {
          ...
          ws.on("message", (data) => {
            ...
            for (const rawFrame of split.frames) {
              ...
              previousText = next;
              if (delta) controller.enqueue(encoder.encode(makeSseChunk(input.model, { content: delta })));
