...
import { errorJson, extractTextFromContent, jsonResponse, withTimeoutSignal } from "./websession-utils.js";
...
function transformDuckStream(body) {
...
async function collectDuckText(response) {
...
export class DuckDuckGoWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, signal }) {
    ...
    const mergedSignal = withTimeoutSignal(signal);
    const headers = {
      Accept: "*/*",
    ...
    const status = await fetch(STATUS_URL, { method: "GET", headers, signal: mergedSignal });
    ...
    const response = await fetch(CHAT_URL, { method: "POST", headers: chatHeaders, body: JSON.stringify(transformedBody), signal: mergedSignal });
    if (!response.ok) {
    ...
    if (stream) return { response: new Response(transformDuckStream(response.body), { headers: { "Content-Type": "text/event-stream" } }), url: CHAT_URL, headers: chatHeaders, transformedBody };
    const content = await collectDuckText(response);
