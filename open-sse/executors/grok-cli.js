...
export function countGrokCliUserTurns(input) {
  ...
  for (const item of input) {
    ...
    const type = isString(item.type) ? item.type : "";
...
function stripStoredItemReferences(body) {
  ...
  body.input = body.input.filter((item) => {
    if (isString(item) && SERVER_ID_PATTERN.test(item)) return false;
    if (item && isObject(item) && !Array.isArray(item)) {
      if (item.type === "item_reference") return false;
      if (isString(item.id) && SERVER_ID_PATTERN.test(item.id)) delete item.id;
    }
    return true;
  });
}
...
function normalizeGrokCliTools(body) {
  ...
  body.tools = body.tools.filter((tool) => {
    ...
    if (type !== "function") {
      ...
      } else if (!type || isString(tool.name)) {
    ...
    const isFunction =
    type === "function" || type === "" || tool.function || isString(tool.name);
    if (!isFunction || HOSTED_TOOL_TYPES.has(type)) {
      return HOSTED_TOOL_TYPES.has(type);
    }

    const fn =
    tool.function && isObject(tool.function) && !Array.isArray(tool.function) ?
    tool.function :
    null;
    const rawName =
    isString(tool.name) ? tool.name : isString(fn?.name) ? fn.name : "";
    ...
    tool.description :
    isString(fn?.description) ?
    fn.description :
    "";
    const parameters =
    tool.parameters && isObject(tool.parameters) && !Array.isArray(tool.parameters) ?
    tool.parameters :
    fn?.parameters && isObject(fn.parameters) && !Array.isArray(fn.parameters) ?
    fn.parameters :
...
function resolveEffortFromModel(modelId) {
  if (!modelId || !isString(modelId)) return null;
...
export class GrokCliExecutor extends BaseExecutor {
  ...
  static formatAgentId(mid) {
    if (!mid || !isString(mid)) return crypto.randomUUID();
  ...
  parseError(response, bodyText) {
    ...
    if (response.status === 402 && bodyText) {
      try {
        ...
        return {
          status: 402,
          message: isString(msg) ? msg : bodyText,
    ...
    if (!body.input || Array.isArray(body.input) && body.input.length === 0) {
      if (Array.isArray(body.messages) && body.messages.length > 0) {
        body.input = body.messages.map((m) => ({
          type: "message",
          role: m.role || "user",
          content: isString(m.content) ? m.content : JSON.stringify(m.content ?? "")
        }));
