 ...
 *
 * Upstream provenance: diegosouzapw/OmniRoute#14649.
 */
import { isObject, isString } from "../../src/shared/utils/typeChecks.js";

const ROOT_UNION_KEYWORDS = ["anyOf", "oneOf"];
...
function typeAllowsObject(type) {
...
function hasRootRelativeRef(node) {
 ...
 */
function mergeRefSiblings(target, siblings) {
 ...
 */
function dereferenceBranch(root, branch, expanded) {
  let current = branch;
  let hops = 0;
  while (isString(current.$ref)) {
 ...
 */
function asObjectBranch(branch) {
  const { type } = branch;
...
function pureUnionBranches(schema) {
  ...
  return pure ? schema[keyword] : null;
}

/** Append the inline object schemas a root union branch stands for to `out`. */
function collectObjectBranches(root, branch, depth, expanded, out) {
...
function mergeSharedKeywordsIntoBranch(branch, shared) {
  ...
  const next = { ...branch };
  if (hasProperties) {
    const ownProperties = isRecord(branch.properties) ? branch.properties : null;
    next.properties = ownProperties ? { ...shared.properties, ...ownProperties } : { ...shared.properties };
  }
...
function normalizeRootUnions(schema) {
  ...
  for (const keyword of keywords) {
    ...
    const kept = collected.map((branch) => mergeSharedKeywordsIntoBranch(branch, shared));
    if (kept.length !== branches.length || kept.some((b, i) => b !== branches[i])) changed = true;
    if (kept.length > 0) next[keyword] = kept;else
    delete next[keyword];
  }
 ...
 */
export function normalizeGrokCliToolSchemas(tools) {
  let changed = false;
