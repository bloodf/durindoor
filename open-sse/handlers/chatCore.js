...
function stripLegacyCompactMarker(body, clientRawRequest) {
  ...
  if (body && isObject(body) && Object.prototype.hasOwnProperty.call(body, "_compact")) {
    cleanBody = { ...body };
    delete cleanBody._compact;
  }
  if (clientRawRequest?.body && isObject(clientRawRequest.body) &&
...
function composeAbortSignals(...signals) {
  const active = signals.filter((signal) => signal && isBoolean(signal.aborted));
...
async function cancelResponseBody(response) {
  ...
  } catch {/* best-effort connection release */} finally
...
export async function handleChatCore({ body, modelInfo, credentials: rawCredentials, log, refreshCredentials, onCredentialsRefreshed, onRequestSuccess, onEmptyStream, onProviderAttempt, quotaReservation = null, abortSignal = null, onDisconnect, onUpstreamEmptyExhausted, clientRawRequest, connectionId, comboId = null, comboName = null, userAgent, apiKey, apiKeyName = "Local (No API Key)", ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, headroomTimeoutMs, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, pxpipeAllowedModels, onPxpipeEvent, onHeadroomEvent, onTokenSaverEvent, sourceFormatOverride, providerThinking, providerConcurrencyLimit, compressionEnabled, compressionEngines, skipPonytailCommands = false, claudeClassifierCompat, modelCapabilities = null }) {
  const credentials = rawCredentials ?
  {
    ...
    { providerSpecificData: { ...rawCredentials.providerSpecificData } } : null)
  ...
  const streamController = createStreamController({
    ...
    onDisconnect: (reason) => {
      ...
      abandonStreamingDetail?.(isString(reason?.reason) ? reason.reason : "client_disconnected");
  ...
  const oauthProxy = proxyData.oauthProxy && isObject(proxyData.oauthProxy) ?
  ...
  if (!executor.noAuth && (parsedError?.statusCode === HTTP_STATUS.UNAUTHORIZED || parsedError?.statusCode === HTTP_STATUS.FORBIDDEN)) {
    try {
      ...
      const newCredentials = isFunction(refreshCredentials) ?
...
export function shouldDefaultAllowClassifier(sourceFormat, body, claudeClassifierCompat) {
  ...
  body.system.map((p) => isString(p?.text) ? p.text : "").filter(Boolean) :
  [];
