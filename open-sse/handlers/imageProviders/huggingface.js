...
const mappingPath = (entry) => (isString(entry) ? entry : entry.path);
const mappingTask = (entry) => (isString(entry) ? "text-to-image" : entry.task || "text-to-image");
...
function customBaseUrl(creds) {
  ...
  return isString(url) && url.trim() ? url.trim().replace(/\/+$/, "") : null;
}
