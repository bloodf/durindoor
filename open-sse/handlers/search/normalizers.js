...
function makeResult(providerId, item, idx, now) {
  ...
  return {
    ...
    position: idx + 1,
    score: isNumber(item.score) ? Math.min(1, Math.max(0, item.score)) : null,
...
function normalizeSerper(data, _query, searchType) {
  ...
  return { results, totalResults: isNumber(total) ? total : null };
}
...
function normalizeGooglePse(data, _query, _searchType) {
  ...
  const total = isString(raw) ? Number(raw) : raw;
...
function normalizeSearchApi(data, _query, _searchType) {
  ...
  const total = isNumber(raw) ? raw : isString(raw) ? Number(raw) : null;
  return { results, totalResults: Number.isFinite(total) ? total : results.length };
}

function normalizeYouCom(data, _query, searchType) {
  const now = new Date().toISOString();
  const container = data?.results && isObject(data.results) ? data.results : undefined;
  const section = searchType === "news" ? container?.news || [] : container?.web || [];
  const items = Array.isArray(section) ? section : [];
  const results = items.map((item, idx) => {
    const firstSnippet = Array.isArray(item.snippets) ? item.snippets.find((v) => isString(v)) : null;
    const livecrawlText = isString(item.markdown) ? item.markdown : isString(item.html) ? item.html : undefined;
    const livecrawlFormat = isString(item.markdown) ? "markdown" : "html";
    return makeResult("youcom", {
      title: item.title,
      url: item.url,
      snippet: isString(firstSnippet) ? firstSnippet : isString(item.description) ? item.description : "",
