...
export function prepareMinimaxVideoRequest(config, { action, requestId, rawBody, contentType }) {
  ...
  if (!config.resolutions.includes(input.resolution)) return badRequest(`MiniMax video resolution must be one of: ${config.resolutions.join(", ")}`);
  if (!Number.isInteger(input.duration) || input.duration < config.duration.min || input.duration > config.duration.max) {
    return badRequest(`MiniMax video duration must be an integer from ${config.duration.min} to ${config.duration.max} seconds`);
  }
