export default {
  ...
  transport: {
    ...
    // Same COSY wire as intl Qoder; reasoning_effort moves into the parameters block.
  ...
  // Same catalog as intl Qoder — CN serves the identical model set from the
  // CN gateway (open-sse/executors/qoder.js resolves the live catalog per
  // credential at chat time; this list is the static/offline fallback).
  models: [
  ...
  oauth: {
    // Not read by any generic consumer today (open-sse/executors/qoder.js and
    // src/lib/oauth/providers.js derive region from the provider id via
    // qoderRegionOf()); kept for operator visibility on this entry.
    region: "cn",
    openApiBaseUrl: "https://openapi.qoder.com.cn",
    // CN collapses center+chat onto one gateway host (no api2/api3 split).
