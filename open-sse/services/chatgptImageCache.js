...
// eviction kicked in. Tune via DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB.
const DEFAULT_MAX_BYTES = 10 * 1024 * 1024;

function configuredMaxBytes() {
  const raw = Number(process.env.DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB);
...
export function storeChatGptImage(bytes, mime, ttlMs = DEFAULT_TTL_MS, context) {
  evictExpired();
...
export function getChatGptImage(id) {
  evictExpired();
  const entry = cache.get(id);
...
export function getChatGptImageConversationContext(id) {
 ...
 */
export function findChatGptImageBySha256(hash) {
  evictExpired();
...
export function __resetChatGptImageCacheForTesting() {
  cache.clear();
  cacheBytes = 0;
}

/** Test-only: peek at current resident-byte total. */
export function __getChatGptImageCacheBytesForTesting() {
  return cacheBytes;
}
