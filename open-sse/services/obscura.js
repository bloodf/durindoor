/**
 * obscura.js — Shared Obscura browser engine (OmniRoute #12274).
 *
 ...
 * module is also disabled entirely when DURINDOOR_BROWSER_POOL=off.
 */
...
export function isObscuraUsable() {
...
function findFreePort() {
  return new Promise((resolve, reject) => {
    ...
    srv.listen(0, "127.0.0.1", () => {
      ...
      srv.close(() => {
        if (address && isObject(address)) resolve(address.port);
...
async function obscuraBinaryPath() {
 ...
 */
export async function connectObscuraBrowser() {
...
export function killSharedObscuraServer() {
  if (shared?.child) {
    try {
