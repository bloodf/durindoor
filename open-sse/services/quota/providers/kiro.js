...
export function normalizeKiroQuota(payload, { accountKey = null, now = Date.now() } = {}) {
  ...
  const plan = typeof data.subscriptionInfo?.subscriptionTitle === "string"
  ...
  for (const raw of asArray(data.usageBreakdownList)) {
    ...
    if (!item || typeof item.resourceType !== "string" || !item.resourceType.trim()) return null;
...
export async function fetchKiroQuota(context) {
  ...
  let profileArn = typeof data.profileArn === "string" && data.profileArn.trim() ? data.profileArn.trim() : null;
  if (!profileArn) {
    ...
    const match = list.find((profile) => typeof profile?.arn === "string" && regionFromProfileArn(profile.arn) === region) || list[0];
    profileArn = typeof match?.arn === "string" && match.arn.trim() ? match.arn.trim() : null;
  }
