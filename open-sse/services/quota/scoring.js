...
export function scoreQuotaCandidate(candidate, {
  ...
  const ratio = isNumber(profile?.effectiveRatio) && Number.isFinite(profile.effectiveRatio) ?
  clamp01(profile.effectiveRatio) :
  null;
  const comparisonKey = isString(profile?.comparisonKey) && profile.comparisonKey.length > 0 ?
