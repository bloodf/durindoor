 ...
 * the cap. This turns the phrasing into `{ requests, windowMs }` so the caller
 * can pace dispatch below it.
 *
 * Ported from OmniRoute's rateLimitManager/requestCap.ts (#13895 / #13594).
 */
...
export function isValidRequestCap(cap) {
  return (
...
/** @returns {{requests: number, windowMs: number} | null} */
export function parseRequestCapFromBody(body) {
