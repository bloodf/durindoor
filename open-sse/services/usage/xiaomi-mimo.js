...
export async function getXiaomiMimoUsage(accessToken = null, providerSpecificData = null, proxyOptions = null) {
  ...
  if (isNumber(account.percent) && Number.isFinite(account.percent)) {
    const remaining = Math.max(0, Math.min(100, Math.round(account.percent)));
    const used = 100 - remaining;
    let resetAt = null;
    if (isNumber(account.resetAt) && account.resetAt > 0) {
      resetAt = new Date(account.resetAt * 1000).toISOString();
    } else if (isString(account.resetDate)) {
  ...
  try {
    ...
    if (!isNumber(percent) || !Number.isFinite(percent)) {
      return {
        plan: "Xiaomi MiMo Desktop",
    ...
    let resetAt = null;
    if (resetDate && isString(resetDate)) {
