...
async function readDesktopAccountCookies() {
  ...
  const tmp = path.join(os.tmpdir(), `durindoor-mimo-cookies-${process.pid}-${crypto.randomBytes(4).toString("hex")}.db`);
  try {
...
async function acquireViaDesktopPhases(passJar, proxyOptions, apiBase, sid) {
  ...
  if (!j || !isNumber(j.code) || j.code !== 0 || !j.location || !j.nonce || !j.ssecurity) {
    failLog(
...
export const __test__ = { sidForRegion, signatureClientSign };
