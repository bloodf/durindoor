...
function createNonStreamingResponse(sourceFormat, model, text) {
  ...
  // For OpenAI Responses API, the Workers handler expects a Responses JSON object
  // and transforms streaming Chat Completions SSE itself, so emit a minimal response
  // shape instead of Chat Completions chunks.
...
function createStreamingResponse(sourceFormat, model, text) {
  ...
  // For the OpenAI Responses API route, the handler transforms Chat Completions SSE
  // into Responses API events, so emit plain OpenAI SSE here.
  if (sourceFormat === FORMATS.OPENAI_RESPONSES) {
    const openaiResponse = createOpenAIResponse(model, text);
    const openaiChunks = createOpenAIStreamingChunks(openaiResponse);
    ...
    for (const chunk of openaiChunks) {
      const formatted = formatSSE(chunk, FORMATS.OPENAI);
      if (formatted) translatedChunks.push(formatted);
    }
