...
//
// The ChatGPT-Web sentinel proof-of-work (open-sse/executors/chatgpt-web.js)
...
// supported` and the provider is unusable on the desktop app (OmniRoute #5531).
//
...
function toBytes(input) {
  return isString(input) ? new Uint8Array(Buffer.from(input, "utf8")) : input;
}

/** Pure-JS SHA3-512 hex digest. Always runs the vendored Keccak (used as the fallback). */
export function sha3_512HexJs(input) {
 ...
 * (Electron/BoringSSL — OmniRoute #5531). Capability is probed once and cached.
 */
export function sha3_512Hex(input) {
...
export function __setSha3NativeForTesting(state) {
  nativeHasher = state;
}
