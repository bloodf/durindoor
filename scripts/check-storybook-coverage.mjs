...
function discover(root) { const result = []; const visit = (dir) => readdirSync(dir, { withFileTypes: true }).forEach((entry) => { const absolute = path.join(dir, entry.name); if (entry.isDirectory()) visit(absolute); else if (SOURCE_EXT.test(entry.name) && !TEST_FILE.test(entry.name) && !entry.name.includes(".stories.")) result.push(path.relative(root, absolute).replaceAll("\\", "/")); }); visit(path.join(root, "src")); return result; }
