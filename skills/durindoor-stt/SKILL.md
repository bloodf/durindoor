---
name: durindoor-stt
description: Speech-to-text via DurinDoor /v1/audio/transcriptions using OpenAI Whisper / Groq / Gemini / Deepgram / AssemblyAI / NVIDIA / HuggingFace models. Use when the user wants to transcribe audio, convert speech to text, or get subtitles from audio files.
---
...
```bash
curl $DURINDOOR_URL/v1/models/stt | jq '.data[].id'
# Per-model params (language, response_format, prompt, temperature support)
curl "$DURINDOOR_URL/v1/models/info?id=openai/whisper-1"
```
...
`POST $DURINDOOR_URL/v1/audio/transcriptions` (OpenAI Whisper compatible, `multipart/form-data`)
...
const r = await fetch(`${process.env.DURINDOOR_URL}/v1/audio/transcriptions`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.DURINDOOR_KEY}` },
  body: form,
});
