...
```bash
curl "$DURINDOOR_URL/v1/models/web" | jq -r '.data[] | select(.kind=="webSearch") | .id'
MODEL_ID="$(curl -s "$DURINDOOR_URL/v1/models/web" | jq -r '.data[] | select(.kind=="webSearch") | .id' | head -n1)"
