...
description: Web search via DurinDoor /v1/search using Tavily / Exa / Brave / Serper / SearXNG / Google PSE / Linkup / SearchAPI / You.com / Perplexity. Use when the user wants to search the web, look up information, find articles, or query a search engine.
---
...
```bash
curl $DURINDOOR_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$DURINDOOR_URL/v1/models/info?id=tavily/search"
```
...
curl -X POST $DURINDOOR_URL/v1/search \
  ...
  -d '{"model":"tavily","query":"DurinDoor open source","max_results":5}'
```

JS:

```js
const r = await fetch(`${process.env.DURINDOOR_URL}/v1/search`, {
  method: "POST",
