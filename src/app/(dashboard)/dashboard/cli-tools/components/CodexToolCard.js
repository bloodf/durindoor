...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_durindoor for localhost if no key, otherwise use selected key
  ...
  const getManualConfigs = () => {
    ...
    const configContent = `# DurinDoor Configuration for Codex CLI
model = "${selectedModel}"
      ...
      <button type="button" aria-expanded={isExpanded} className="w-full min-h-11 text-left outline-none focus-visible:shadow-dd-focus flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
        ...
        <div className="flex min-w-0 items-center gap-3">
          <div className="size-8 flex items-center justify-center shrink-0">
            <Image src="/providers/codex.png" alt={tool.name} width={32} height={32} className="size-8 object-contain rounded-dd-lg" sizes="32px" onError={(e) => { e.target.style.display = "none"; }} />
          </div>
              ...
              <div className="flex flex-col gap-3 p-4 bg-dd-warning/10 border border-dd-warning/30 rounded-dd-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-dd-warning">Codex CLI not detected locally</p>
                ...
                <div className="p-4 bg-dd-surface border border-dd-border rounded-dd-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-dd-surface-2 bg-dd-surface-2 rounded-dd font-mono text-xs">npm install -g @openai/codex</code>
                    </div>
                    <p className="text-dd-muted">After installation, run <code className="px-1 bg-dd-surface-2 bg-dd-surface-2 rounded-dd">codex</code> to verify.</p>
                    <div className="pt-2 border-t border-dd-border">
                      <p className="text-dd-muted text-xs">
                        Codex uses <code className="px-1 bg-dd-surface-2 bg-dd-surface-2 rounded-dd">~/.codex/auth.json</code> with <code className="px-1 bg-dd-surface-2 bg-dd-surface-2 rounded-dd">OPENAI_API_KEY</code>.
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    ...
                    />
                    {subagentModel && (
                      <IconButton icon="close" label="Clear subagent model" size="sm" onClick={() => setSubagentModel("")} className="absolute right-1 top-1/2 -translate-y-1/2 text-dd-muted hover:text-dd-danger" />
                    )}
                  </div>
                  <Button
