            ...
            <div className="flex min-w-0 flex-1 flex-col gap-1 sm:max-w-md">
              ...
              <div ref={modelMenuRef} className="relative min-w-0">
                <button
                  ...
                  aria-activedescendant={modelMenuOpen && activeModelIndex >= 0 ? `${modelListboxId}-opt-${activeModelIndex}` : undefined}
                ...
                {modelMenuOpen ? <Card padding={false} className="absolute start-0 top-[calc(100%+8px)] z-30 w-[min(520px,calc(100vw-1.5rem))] shadow-dd-elevated">
                  ...
                  <div role="listbox" id={modelListboxId} tabIndex={-1} onKeyDown={handleModelListKeyDown} aria-label="Available models" className="max-h-[60vh] space-y-2 overflow-y-auto p-2">
                    {providerGroups.map((group) => <div key={group.providerId} className="rounded-dd border border-dd-border-subtle bg-dd-bg p-2">
...
<Button variant="primary" size="sm" icon="add" onClick={handleNewChat} disabled={!activeModel}>New chat</Button>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          <div className="flex-1 overflow-y-auto px-3 py-4 sm:px-4 lg:px-6">
            {currentMessages.length === 0 ? <EmptyState icon="forum" title="Playground" message={activeModel ? `Chat with ${activeProviderGroup?.providerName || activeModel.providerName} · ${activeModel.name}` : "Chat against connected provider models after they load."} /> : null}
