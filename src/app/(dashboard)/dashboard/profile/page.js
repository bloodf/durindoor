...
const SETTINGS_TABS = [
  ...
  { value: "system", label: "System & Data", icon: "storage" },
];
...
export default function ProfilePage() {
  ...
  // Keep OIDC disclosure unavailable until saved settings settle its layout and draft.
  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-5 px-4 py-6 sm:px-6" aria-label="Profile settings">
      ...
      <div className="[&>[role=tablist]]:flex-wrap" aria-label="Settings categories">
        <Tabs tabs={SETTINGS_TABS} value={activeTab} onChange={setActiveTab} aria-label="Settings categories" />
      </div>

      <section role="tabpanel" aria-label="General settings" hidden={activeTab !== "general"} className="flex flex-col gap-5">

      <Card padding={false}><CardHeader icon="palette" title="Appearance" subtitle="Dashboard display preferences" /><CardContent><Field group label="Theme" hint="Choose how DurinDoor looks on this device."><SegmentedControl aria-label="Theme" options={[{value:"light",label:"Light"},{value:"dark",label:"Dark"},{value:"system",label:"System"}]} value={theme} onChange={setTheme} /></Field></CardContent></Card>
      ...
      <section role="tabpanel" aria-label="System and data settings" hidden={activeTab !== "system"} className="flex flex-col gap-5">
      <Card padding={false}><CardHeader icon="storage" title="Data & Backups" subtitle="Local database, transfer, and backups" /><CardContent className="flex flex-col gap-5">
