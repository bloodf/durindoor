...
import { Card, Button, Badge, Input, Modal, CardSkeleton, OAuthModal, KiroOAuthWrapper, CursorAuthModal, ImportTokenModal, IFlowCookieModal, GitLabAuthModal, GheCopilotAuthModal, Toggle, EditConnectionModal, NoAuthProxyCard, ConfirmModal, ProviderIcon, OrcaRouterAuthModal, OrcaModelDropdown } from "@/shared/components";
...
export default function ProviderDetailPage() {
  ...
  const triggerOAuthConnection = () => {
    if (providerId === "antigravity" && isBrowser()) {
  ...
  const providerInfo = providerNode ?
  {
    ...
    ...(providerNode.iconUrl ? { iconUrl: providerNode.iconUrl } : null)
  } :
  ...
  const saveRetryDelay = async (value) => {
    try {
      ...
      if (value === "auto") delete updated[providerId];else
  ...
  const saveRpmLimit = async (value) => {
    try {
      ...
      if (!trimmed) delete updated[providerId];else
      updated[providerId] = Math.min(MAX_PROVIDER_RPM, Math.max(0, Number.parseInt(trimmed, 10) || 0));
      await fetch("/api/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleAddCustomModel = async (payload, type = "llm", providerAliasOverride = providerStorageAlias) => {
    const modelId = isString(payload) ? payload : payload.id;
    const capabilities = isObject(payload) && payload !== null && !Array.isArray(payload) ? payload.capabilities : undefined;
    try {
      ...
      if (res.ok) {
        ...
        if (isBrowser()) window.dispatchEvent(new CustomEvent("customModelChanged"));
        return;
      }
  ...
  <div role="region" aria-label="Connections" tabIndex={0} className="flex min-w-0 flex-col divide-y divide-black/[0.03] dark:divide-white/[0.03] max-h-[500px] overflow-y-auto pr-1">
      {connections.
    map((conn, index) =>
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6 px-1 sm:gap-8 sm:px-0">
      ...
      <Card>
        ...
        <>
              {oneByOneSummary &&
          <div className="mb-4 rounded-dd bg-dd-surface-2 px-3 py-2 text-xs text-dd-muted">
              ...
              {connections.length > 0 &&
          <div className="mb-3 flex items-center gap-2 border-b border-dd-border-subtle pb-2">
