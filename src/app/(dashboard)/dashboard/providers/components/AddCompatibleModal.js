...
function AddCompatibleModal({ variant, isOpen, onClose, onCreated }) {
  ...
  const initialFormData = () => ({
    name: "",
    prefix: "",
    ...(config.hasApiType ? { apiType: "chat" } : null),
    ...
    try {
      const res = await fetch("/api/provider-nodes", {
        ...
        body: JSON.stringify({
          ...
          ...(config.hasApiType ? { apiType: formData.apiType } : null),
    ...
    if (valid) {
      return (
        <>
          ...
          {method === "chat" ? <span className="text-sm text-dd-muted">(via inference test)</span> : null}
        </>
      );
    }
    return (
      <div className="flex flex-col gap-1">
        ...
        {error ? <span className="text-sm text-dd-danger">{error}</span> : null}
      </div>
    );
  };

  return (
    <Modal open={isOpen} title={config.title} onClose={onClose}>
