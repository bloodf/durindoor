 ...
 * the browser a same-origin pageUrl carrying the dd_mimo_login session cookie.
 * All subsequent account.xiaomi.com traffic flows through src/proxy.js.
 *
 * Egress resolution: MIMO_LOGIN_PROXY env > (non-CN cluster: probe common LOCAL
 * HTTP proxy ports — v2rayN/clash defaults) > direct. CN is always direct. The resolved URL rides
...
const LOGIN_REGIONS = new Set(["cn", "sgp", "ams", "ru", "in"]);
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      // Known MiMo Desktop clusters — default cn.
      if (LOGIN_REGIONS.has(r)) region = r;
    ...
    const envProxy = process.env.MIMO_LOGIN_PROXY?.trim();
    if (region !== "cn" && envProxy) {
      egress = envProxy;
      egressSource = "env";
    } else if (region !== "cn") {
