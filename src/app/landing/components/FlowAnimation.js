...
export default function FlowAnimation() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (motion.matches) return setActiveFlow(0);
      interval = setInterval(() => setActiveFlow((prev) => (prev + 1) % PROVIDERS.length), 2000);
    };
    update();
    motion.addEventListener("change", update);
    return () => { clearInterval(interval); motion.removeEventListener("change", update); };
  }, []);
  ...
  return <>
    ...
    <div className="relative mt-16 hidden h-[360px] w-full max-w-4xl animate-[dd-float_6s_ease-in-out_infinite] items-center justify-center md:ltr:flex md:rtl:hidden motion-reduce:animate-none">
      <div className="group relative z-20 flex h-32 w-32 cursor-pointer flex-col items-center justify-center gap-1 rounded-full border-2 border-dd-accent bg-dd-surface shadow-dd-elevated transition-transform duration-500 hover:scale-105"><span aria-hidden="true" className="absolute inset-0 rounded-full border border-dd-accent/30 opacity-20 animate-ping motion-reduce:animate-none" /><span aria-hidden="true" className="relative material-symbols-outlined text-4xl text-dd-accent">hub</span><span className="relative text-[10px] font-bold uppercase tracking-widest text-dd-text">DurinDoor</span></div>
      ...
      <svg className="pointer-events-none absolute inset-0 z-0 h-full w-full" viewBox="0 0 800 360" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
        {[50, 140, 210, 300].map((y) => <path key={`in-${y}`} d={`M 60 ${y} C 250 ${y}, 250 180, 360 180`} fill="none" stroke="var(--dd-accent-2)" strokeWidth="2" strokeDasharray="5,5" className="animate-[dd-dash_2s_linear_infinite] motion-reduce:animate-none" opacity="0.5" />)}
        {[50, 130, 230, 310].map((y, idx) => <path key={`out-${idx}`} d={`M 440 180 C 550 180, 550 ${y}, 740 ${y}`} fill="none" stroke={activeFlow === idx ? "var(--dd-accent)" : "var(--dd-border)"} strokeWidth={activeFlow === idx ? "3" : "2"} className={activeFlow === idx ? "animate-pulse motion-reduce:animate-none" : ""} />)}
      </svg>
