...
export default function LandingPage() {
  ...
  return (
    <div className="relative overflow-x-hidden bg-dd-bg font-sans text-dd-text antialiased selection:bg-dd-accent selection:text-dd-on-accent">
      ...
      <div className="relative z-10"><Navigation /><main><div className="relative"><HeroSection /><div className="flex justify-center pb-20"><FlowAnimation /></div></div><GetStarted /><HowItWorks /><Features /><section className="relative overflow-hidden bg-dd-accent-soft px-6 py-32"><div className="relative z-10 isolate mx-auto max-w-4xl rounded-dd-lg bg-dd-surface px-6 py-10 text-center shadow-dd-elevated sm:px-10 md:py-12"><h2 className="mb-6 text-4xl font-black leading-tight text-dd-text md:text-5xl">Ready to simplify your AI infrastructure?</h2><p className="mx-auto mb-10 max-w-2xl text-xl leading-relaxed text-dd-muted">Join developers streamlining AI integrations with DurinDoor. Open source and free to start.</p><div className="flex flex-col items-center justify-center gap-4 sm:flex-row"><Button variant="primary" className="w-full sm:w-auto" onClick={() => router.push("/dashboard")}>Start free</Button><a href="https://github.com/bloodf/durindoor#readme" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 w-full items-center justify-center rounded-dd border border-dd-border bg-dd-surface-2 px-3.5 text-[13px] font-medium text-dd-text outline-none transition-colors hover:bg-dd-surface-3 focus-visible:shadow-dd-focus sm:w-auto">Read documentation</a></div></div></section></main><Footer /></div>
