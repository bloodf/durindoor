 ...
 * each is seeded with the matching prefixed DurinDoor model so routing works.
 */
...
export function isValidComboPresetName(name) {
  return isString(name) && name.length > 0 && VALID_COMBO_NAME_REGEX.test(name);
}
 ...
 * Prefer a caller-supplied live catalog when provided; otherwise the static cu registry.
