...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: db.all(`SELECT * FROM apiKeys`).map((r) => { let ac = []; try { ac = JSON.parse(r.allowedCombos); if (!Array.isArray(ac)) ac = []; } catch {} return { id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1, allowedCombos: ac, dailyLimitTokens: r.dailyLimitTokens ?? null, policy: parseJson(r.policy, { allowedModels: [] }), createdAt: r.createdAt }; }),
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    for (const k of payload.apiKeys || []) {
      db.run(
        `INSERT OR REPLACE INTO apiKeys(id, key, name, machineId, isActive, allowedCombos, dailyLimitTokens, policy, createdAt) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [k.id, k.key, k.name || null, k.machineId || null, k.isActive === false ? 0 : 1, JSON.stringify(k.allowedCombos || []), k.dailyLimitTokens ?? null, stringifyJson(k.policy || { allowedModels: [] }), k.createdAt || new Date().toISOString()]
      );
    }
