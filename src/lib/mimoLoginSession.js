 ...
 *      travels in the httpOnly dd_mimo_login cookie between hops).
...
// Paths that belong to the DurinDoor app itself — never proxy these upstream,
...
const APP_PREFIXES = [
  "/_next/", "/api/", "/dashboard", "/v1", "/v1beta", "/codex", "/responses",
  "/login", "/landing", "/callback", "/__mimo_login/", "/i18n/", "/icons/", "/providers/", "/fonts/",
];
const APP_FILES = new Set([
  "/", "/favicon.svg", "/favicon.ico", "/file.svg", "/globe.svg", "/next.svg",
  "/vercel.svg", "/window.svg", "/sw.js", "/robots.txt", "/manifest.webmanifest",
  "/apple-touch-icon.png", "/durindoor-wordmark.png", "/theme-bootstrap.js",
]);
...
export function originOf(request) {
  ...
  return host ? `${proto}//${host}` : (request.nextUrl?.origin || "http://localhost:20128");
}
...
export function decodeSessionCookie(value) {
  ...
  if (!payload || !isNumber(payload.t)) return null;
  ...
  return {
    ...
    proxyUrl: isString(payload.p) && payload.p ? payload.p : null,
    jar,
    status: "pending",
    createdAt: payload.t,
...
async function socksFetch(url, init, proxyUrl) {
  ...
  let body = init?.body;
  if (body && !isString(body) && !Buffer.isBuffer(body)) body = Buffer.from(body);
...
export async function proxyAccountRequest(sess, req, origin) {
  ...
  for (let hop = 0; hop < 8; hop++) {
    ...
    if (res.status >= 300 && res.status < 400 && loc) {
      ...
      if (/^mimo-server-(cn|sgp|ams|ru|in)\.xiaomimimo\.com$/.test(nextAbs.hostname)) {
...
function donePage() {
  return htmlPage("Signed in", ["The Xiaomi account session was captured. You can close this window and continue in DurinDoor."], true);
}

function pendingPage() {
  return htmlPage("Sign-in not finished", ["No account session was found. Please try again."]);
}
