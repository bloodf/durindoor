...
export default function UiVersionSwitch() {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p className="text-[13px] font-medium text-dd-text">Dashboard version</p>
      ...
      <div role="group" aria-label="Dashboard version" className="flex shrink-0 gap-1 rounded-dd border border-dd-border bg-dd-surface-2 p-1">
        {[
          ...
          <button
            ...
            disabled={pending}
            className={`min-h-11 rounded-dd px-3 text-[13px] font-medium outline-none transition-colors focus-visible:shadow-dd-focus disabled:opacity-60 ${
              version === option.value
                ? "bg-dd-accent text-dd-on-accent"
                : "text-dd-text hover:bg-dd-surface-3"
            }`}
          >
            {option.label}
          </button>
