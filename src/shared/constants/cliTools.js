...
export const MITM_TOOLS = {
  antigravity: {
    ...
    modelAliases: ["gemini-3.8-flash-high", "gemini-3.8-flash-medium", "gemini-3.8-flash-low", "gemini-3.7-flash-high", "gemini-3.7-flash-medium", "gemini-3.7-flash-low", "gemini-3.5-flash-low", "gemini-3-flash-agent", "gemini-3.5-flash-extra-low", "gemini-3.1-pro-low", "gemini-pro-agent", "claude-sonnet-4-6", "claude-opus-4-6-thinking", "gpt-oss-120b-medium", "gemini-3-flash"],
    defaultModels: [
  ...
  kiro: {
    ...
    defaultModels: [
      ...
      // decolua/9router#2596 — GPT-5.6 family picker slots. Static rows (not
      // imported from open-sse/config/providerModels.js): this file ships in
      // the shared/client bundle and must not pull the server config graph.
...
export const CLI_TOOLS = {
  ...
  amp: {
    ...
    configType: "guide",
    /** Amp cannot route model inference through a custom OpenAI-compatible base URL. */
    unsupported: true,
    notes: [
      { type: "error", text: "Amp does not support custom OpenAI-compatible base URLs. AMP_URL configures the Amp service, not the model inference endpoint, so Amp cannot connect directly to DurinDoor." },
    ],
  },
  qwen: {
    id: "qwen",
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via DurinDoor",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. DurinDoor works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in DurinDoor can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use DurinDoor with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. DurinDoor will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure durindoor as an OpenAI-compatible provider to route all jcode requests through durindoor's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. DurinDoor writes a [model.9router] custom model (display name DurinDoor) and sets it as the default.",
      },
      {
        type: "info",
