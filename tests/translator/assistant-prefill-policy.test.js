...
function passthrough(messages, headers = null) {
  return normalizeClaudePassthrough({
    model: "claude-sonnet-4-6",
    messages: structuredClone(messages),
  }, "claude-sonnet-4-6", "claude", null, { rawHeaders: headers });
}
...
describe.each([
  ...
  it("completes every id-bearing trailing tool_use with an error tool_result", () => {
    const out = run([
      { role: "user", content: "Start" },
      { role: "assistant", content: [
  ...
  it("keeps a final user turn unchanged", () => {
  ...
  it("preserves assistant prefill when explicitly requested", () => {
    ...
    ], new Headers({ "X-9Router-Assistant-Prefill": "preserve" }));
