...
describe("Alibaba Token Plan provider", () => {
  ...
  it("does not collide with the other Alibaba key types present in this fork", () => {
    // ponytail: alims-intl (Model Studio split out of alicode-intl) isn't in
    // this lineage yet — Model Studio still lives under alicode-intl here.
    const hosts = ["alicode", "alicode-intl", "alitp-intl"]
      .map((id) => new URL(PROVIDERS[id].baseUrl).host);
    expect(new Set(hosts).size).toBe(hosts.length);
  });

  it("exposes exactly the six models the plan serves, in registry order", () => {
