...
describe("ChatGPT Web executor", () => {
  ...
  it("resolves relative ChatGPT script URLs before building Sentinel PoW config", async () => {
    const calls = [];
    globalThis.fetch = vi.fn(async (url, opts = {}) => {
      calls.push({ url: String(url), opts });
      if (String(url).endsWith("/api/auth/session")) {
        return new Response(JSON.stringify({
          accessToken: "access-token-relative-script",
          expires: new Date(Date.now() + 60000).toISOString(),
        }), { status: 200 });
      }
      if (String(url) === "https://chatgpt.com/") {
        return new Response('<html data-build="prod-rel"><script src="/_next/static/chunks/app.js"></script></html>', { status: 200 });
      }
      if (String(url).endsWith("/backend-api/sentinel/chat-requirements/prepare")) {
        return new Response(JSON.stringify({ prepare_token: "prepare-token-rel" }), { status: 200 });
      }
      if (String(url).endsWith("/backend-api/sentinel/chat-requirements")) {
        return new Response(JSON.stringify({ token: "requirements-token-rel", proofofwork: { required: false } }), { status: 200 });
      }
      return sseResponse([{ message: { status: "finished_successfully", content: { parts: ["relative ok"] } } }]);
    });
    ...
    await new ChatGptWebExecutor().execute({
      ...
      body: { messages: [{ role: "user", content: "hi" }] },
      stream: false,
      credentials: { apiKey: "__Secure-next-auth.session-token=session-relative-script" },
    });

    const prepare = calls.find((call) => call.url.endsWith("/backend-api/sentinel/chat-requirements/prepare"));
    const token = JSON.parse(prepare.opts.body).p;
    const config = JSON.parse(Buffer.from(token.slice("gAAAAAC".length), "base64").toString("utf8"));
    expect(config[5]).toBe("https://chatgpt.com/_next/static/chunks/app.js");
  });
