...
} from "../../open-sse/utils/chatgptWebBrowserSession.js";
const HANDOFF_SSE =
  ...
  "data: [DONE]\n\n";
function streamItem(id, encodedItem, topicId = "topic") {
...
function doneFrame(topicId = "topic") {
  return JSON.stringify([
    {
      type: "message",
      topic_id: topicId,
...
class FakeBrowserSession {
  ...
  constructor(
    execute,
    sessionUrl = "https://chatgpt.com/?temporary-chat=true",
    renderedAssistantText = null
  ) {
    this.execute = execute;
    this.sessionUrl = sessionUrl;
    this.renderedAssistantText = renderedAssistantText;
  }
  url() {
...
describe("ChatGPT Web clean-room browser-owned session", () => {
  test("decodes a direct first-party conversation response without DOM or WebSocket handoff", async () => {
    ...
    assert.equal(submitted.prompt, "direct prompt");
