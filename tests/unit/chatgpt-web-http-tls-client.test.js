 ...
 *   2. DURINDOOR_TLS_PROXY_URL env var (single-flag opt-in).
...
import { tlsFetchChatGpt, __setTlsFetchOverrideForTesting } from "../../open-sse/services/chatgptTlsClient.js";

const PROXY_ENV_KEYS = [
  "DURINDOOR_TLS_PROXY_URL",
  "HTTPS_PROXY",
  "https_proxy",
  "HTTP_PROXY",
  "http_proxy",
...
describe("chatgptTlsClient — proxy plumbing (#2022)", async () => {
  ...
  it("per-call proxyUrl overrides everything", async () => {
    process.env.DURINDOOR_TLS_PROXY_URL = "http://env-omni:0/";
    ...
    expect(observedOpts.proxyUrl).toBe("http://per-call:0/");
  });
