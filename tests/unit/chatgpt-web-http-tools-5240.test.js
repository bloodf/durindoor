...
function convWithAssistantText(parts) {
  return [
    {
...
function installMockFetch(convEvents) {
...
test("Tools request-side: <tool> contract is serialized into the upstream system message (#5240)", async () => {
  ...
  try {
    ...
    const convBody = JSON.parse(m.calls.bodies[convIdx]);
    const systemMsg = convBody.messages.find((mm) => mm.author.role === "system");
