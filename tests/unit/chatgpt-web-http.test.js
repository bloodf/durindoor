...
const { ChatGptWebHttpExecutor, __derivePublicBaseUrlForTesting, __resetChatGptWebCachesForTesting } =
  await import("../../open-sse/executors/chatgpt-web-http.js");
const { describeChatGptWebHttpError } =
  await import("../../open-sse/executors/chatgptWebErrors.js");
const {
  __setTlsFetchOverrideForTesting,
  __tlsFetchStreamingForTesting,
  looksLikeSse,
  TlsClientUnavailableError,
} = await import("../../open-sse/services/chatgptTlsClient.js");
...
test("ChatGptWebHttpExecutor sets correct provider name", () => {
  const executor = new ChatGptWebHttpExecutor();
  assert.equal(executor.getProvider(), "chatgpt-web");
});

// ─── Public image URL derivation ────────────────────────────────────────────

test("Image URL base: DURINDOOR_PUBLIC_BASE_URL wins and strips accidental /v1", async () => {
  await withEnv(
    {
      DURINDOOR_PUBLIC_BASE_URL: " http://192.168.107.55:20128/v1/ ",
...
test("Image URL base: non-local BASE_URL remains a compatibility fallback", async () => {
  await withEnv({ BASE_URL: "https://omni.example.com/v1" }, async () => {
...
test("Executor MODEL_MAP: catalog IDs translate to ChatGPT backend slugs", async () => {
  reset();
  const m = installMockFetch();
  try {
    const cases = [
      ...
      ["gpt-5-6", "gpt-5-6"],
      // Registry ids shared with the browser transport.
      ["gpt-5-5-instant", "gpt-5-5"],
...
test("GPT-5.6 Luna Free Think sends the captured auto-router reason hints", async () => {
  ...
  try {
    ...
    for (const [model, expectedHints] of [
      ...
      const convIdx = m.calls.urls.findIndex((u) => u.endsWith("/backend-api/f/conversation"));
      const body = JSON.parse(m.calls.bodies[convIdx]);
      const userMessage = body.messages.find((message) => message.author?.role === "user");
...
test("Image gen: signed URL bytes are cached and exposed via /v1/chatgpt-web/image URL", async () => {
  ...
  const tls = await import("../../open-sse/services/chatgptTlsClient.js");
  __resetChatGptWebCachesForTesting();
  ...
  await withEnv(
    ...
    async () => {
      try {
        ...
        const cacheMod = await import("../../open-sse/services/chatgptImageCache.js");
...
test("Image edit: cached OmniRoute image URL continues the saved ChatGPT conversation", async () => {
  reset();
  const { storeChatGptImage } = await import("../../open-sse/services/chatgptImageCache.js");
...
test("Image cache: byte cap evicts oldest before count cap kicks in", async () => {
  ...
  const original = process.env.DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB;
  process.env.DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB = "10";
  try {
    ...
    if (original == null) delete process.env.DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB;
    else process.env.DURINDOOR_CGPT_WEB_IMAGE_CACHE_MAX_MB = original;
    cacheMod.__resetChatGptImageCacheForTesting();
  }
});
