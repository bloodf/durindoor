...
import { verifyApiKeyExpiryColumnLayout } from "../../src/lib/db/helpers/schemaVerifier.js";

describe("published schema preflight", () => {
  it("accepts a missing table/column and the published nullable TEXT shape", () => {
    const db = new Database(":memory:");
    expect(() => verifyApiKeyExpiryColumnLayout({ get: (sql, params) => db.prepare(sql).get(params), all: (sql) => db.prepare(sql).all() })).not.toThrow();
  ...
  it.each([
    ...
    expect(() => verifyApiKeyExpiryColumnLayout(adapter)).toThrow("Published schema mismatch");
    db.close();
  });
});
